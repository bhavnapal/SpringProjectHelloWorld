					...
					$(document)
							.on(
									...
									function() {
										...
										var jobDescId = $(this).parent().find(
												'input').val();
										...
										$
												.ajax({
													type : "DELETE",
													url : window.location
															+ "jobDesc/deleteJobRequestListId/"
															+ jobDescId,
													success : function(
															resultMsg) {
														$("#resultMsgDiv")
																.html(
																		"<p style='text-align:centre; background-color:#67597E; color:white; padding:20px 20px 20px 20px'>"
																				+ "JobDescription with Id: "
																				+ jobDescId
																				+ " is Deleted successfully!"
																				+ "</p>");

														workingObject.closest(
																"tr").remove();
										...
										$
												.ajax({
													...
													success : function(result) {
														...
														console
																.log(result.jobDescId);

														$("#raisedonDate")
																.append(
																		result.jobDescRaisedOnDate);
														$("#jobDescId")
																.append(
																		result.jobDescId);
														$("#profile")
																.append(
																		result.jobDescProfile);
														$("#project")
																.append(
																		result.jobDescProject);
														$("#experience")
																.append(
																		result.jobDescExperience);
														$("#qualification")
																.append(
																		result.jobDesQualification);
														$("#certification")
																.append(
																		result.jobDescCertification);
														$("#skill_mand")
																.append(
																		result.jobDescSkillsSetMandatory);
														$("#skill_opt")
																.append(
																		result.jobDescSkillsSetOptional);
														$("#responsibility")
																.append(
																		result.jobDescResponsibility);
														$("#expected_doj")
																.append(
																		result.jobDescExpectedDateOfJoining);
			...
			.ajax({
				...
				success : function(result) {
					$
							.each(
									result,
									function(index, JobDecBo) {

										var customerRow = '<tr>' + '<td>'
												+ JobDecBo.jobDescId
												+ '</td>'
												+

												'<td>'
												+ JobDecBo.jobDescProfile
												+ '</td>'
												+

												'<td>'
												+ JobDecBo.jobDescEmployee
												+ '</td>'
												+

												'<td>'
												+ JobDecBo.jobDescRaisedOnDate
												+ '</td>'
												+
...
function addJobDescRecq() {
	...
	console.log(jobDescRaisedOnDate + " " + jobDescProfile + " "
			+ jobDescProject + "" + jobDescExperience + ""
			+ jobDesQualification + "" + jobDescCertification + ""
			+ jobDescCertification + "" + jobDescSkillsSetMandatory + ""
			+ jobDescSkillsSetOptional + "" + jobDescResponsibility + ""
			+ jobDescExpectedDateOfJoining);
	...
	$.ajax({
		...
		success : function(txt) {
			...
			/*
			 * $.ajax({ type : "GET", url : window.location +
			 * "jobDesc/getJobRequestListHome", success: function(result){
			 * $.each(result, function(index, JobDecBo){
			 * 
			 * var customerRow = '<tr>' + '<td>'+ JobDecBo.jobDescId+ '</td>' + '<td>' +
			 * JobDecBo.jobDescProfile+ '</td>' + '<td>' +
			 * JobDecBo.jobDescEmployee+'</td>' + '<td>' +
			 * JobDecBo.jobDescRaisedOnDate+'</td>' +
			 * 
			 *  '<td class="text-center">' + '<input type="hidden" value=' +
			 * JobDecBo.jobDescId + '>' + '<a>' + '<span class="glyphicon
			 * glyphicon-eye-open"></span>' + '</a>&nbsp;&nbsp;&nbsp;' + '<a>' + '<span
			 * class="glyphicon glyphicon-pencil"></span>' + '</a>&nbsp;&nbsp;&nbsp;' + '<a>' + '<span
			 * class="glyphicon glyphicon-remove"></span>' + '</a>&nbsp;&nbsp;&nbsp;' +
			 * 
			 *  '</td>' + '<td>' + JobDecBo.jobDescStatus+ '</td>' + '</tr>';
			 * 
			 * $('#example tbody').append(customerRow);
			 * 
			 * }); $( "#example tbody tr:odd" ).addClass("info"); $( "#example
			 * tbody tr:even" ).addClass("success"); }, error : function(e) {
			 * alert("ERROR: ", e); console.log("ERROR: ", e); } });
			 */console.log(x);
